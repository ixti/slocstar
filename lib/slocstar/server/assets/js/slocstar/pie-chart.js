...
;(function ($) {
  ...
  SlocStar.initPieChart = function (size) {
    var $el, paper, x, y, r, a, i, mid;
    ...
    // middle spot
    mid = paper.circle(x, y, r * 0.20).attr({
      "fill": "white",
      "fill-opacity": 0.7,
      "stroke": "black",
      "stroke-width": 1,
    ...
    $el.sector = function (rate, title) {
      var turn = 360 * rate, s, t;
      ...
      t = paper.text(x, y, title).attr({
        "fill": Raphael.hsb(i, 0.8, 0.5),
        "stroke": "none",
        "opacity": 0,
      ...
      s.activate = function () {
        ...
        mid.stop().animate({opacity: 1}, 500, "easeOut");
        t.stop().animate({opacity: 1}, 500, "easeOut");
      };

      s.deactivate = function () {
        s.stop().animate({transform: ""}, 500, "elastic");
        mid.stop().animate({opacity: 0}, 500);
        t.stop().animate({opacity: 0}, 500);
      };
